...
function paintText() {
  ...
  let letterSize = map(mouseX, 0, width, 12, 128);
  let letterColor = map(mouseY, 0, height, 255, 0);

  const cols = floor(width / letterSize);
  const rows = floor(height / letterSize);
  
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      // Calculate cell position
      const xPos = x * letterSize + letterSize / 2;
      const yPos = y * letterSize + letterSize / 2;
    ...
    for (let x = 0; x < cols; x++) {
      ...
      textSize(letterSize);10
      fill(360 - hue, 50, 150, 0.5);
